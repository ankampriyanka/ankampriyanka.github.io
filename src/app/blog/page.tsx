    ...
    <main className="min-h-screen bg-white py-24 text-slate-900 dark:bg-slate-950 dark:text-white">
      <Container>
        ...
        <section aria-labelledby="research-articles" className="grid gap-6 md:grid-cols-2">
          <h2 id="research-articles" className="sr-only">
            Research articles
          </h2>
          ...
          {articles.map((article) => (
            <article
              key={article.href}
              className="group rounded-2xl border border-slate-200 bg-white p-7 shadow-sm transition hover:-translate-y-1 hover:shadow-md dark:border-slate-800 dark:bg-slate-900"
            >
              ...
              <div className="mt-6 flex items-center justify-between text-sm text-slate-500 dark:text-slate-400">
                ...
                <Link href={article.href} className="font-semibold text-slate-900 dark:text-white">
                  Read article →
                </Link>
              </div>
            </article>
