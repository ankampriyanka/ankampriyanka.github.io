      ...
      <Container>
        <article className="mx-auto max-w-4xl">
          <header className="mb-10 border-b border-slate-200 pb-9 dark:border-[#C0CAD6]">
            ...
            <p className="mb-3 font-sans text-xs font-semibold text-slate-500 dark:text-slate-600">PART 3 · AI GOVERNANCE DEBT SERIES</p>
          ...
          <div className="font-serif text-base leading-7 text-slate-700 dark:text-slate-600 sm:text-[17px] sm:leading-8">
            ...
            <section className="mb-12 grid gap-5 md:grid-cols-3">
              {debts.map((debt, index) => (
                <article key={debt.name} className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-[#C0CAD6] dark:bg-[#EBEEF3]">
                  <div className="mb-4 flex h-8 w-8 items-center justify-center rounded-full bg-slate-100 font-sans text-xs font-bold text-slate-600 dark:bg-slate-800 dark:text-slate-600">
                    {index + 1}
                  </div>
                  <h2 className="font-sans text-lg font-bold leading-7 text-slate-950 dark:text-[#4E537D]">{debt.name}</h2>
                  <p className="mt-3 text-sm leading-6 text-slate-600 dark:text-slate-600">{debt.question}</p>
                </article>
              ))}
            </section>
            ...
            <section className="mb-12 border-y border-slate-200 py-10 dark:border-[#C0CAD6]">
              <p className="text-lg font-semibold leading-8 text-slate-950 dark:text-[#4E537D] sm:text-xl">
            ...
            <section className="mb-12">
              ...
              <ol className="space-y-5 pl-6">
                <li className="pl-2">
                  <strong className="font-sans font-bold text-slate-950 dark:text-[#4E537D]">It&apos;s portfolio-wide.</strong> An organization may run hundreds of AI systems, models and third-party AI services — each accumulating its own gaps.
                </li>
                <li className="pl-2">
                  <strong className="font-sans font-bold text-slate-950 dark:text-[#4E537D]">It&apos;s dynamic.</strong> Models change, regulations change, use cases change, risk profiles change — governance has to track a moving target.
                </li>
                <li className="pl-2">
                  <strong className="font-sans font-bold text-slate-950 dark:text-[#4E537D]">It compounds.</strong> If governance capability doesn&apos;t scale as fast as the AI portfolio grows, unresolved obligations pile up faster than the organization can close them.
                </li>
              </ol>
            </section>
            ...
            <section className="mb-12">
              <h2 className="mb-5 font-sans text-xl font-bold tracking-tight text-slate-950 dark:text-[#4E537D] sm:text-2xl">
                A simple management model
              </h2>
              <div className="overflow-hidden rounded-2xl border border-slate-200 dark:border-[#C0CAD6]">
                <div className="grid grid-cols-2 bg-slate-50 font-sans text-xs font-bold uppercase tracking-wider text-slate-500 dark:bg-[#EBEEF3] dark:text-slate-600">
                  <div className="px-5 py-3">Debt type</div>
                  <div className="px-5 py-3">Management concern</div>
                </div>
                {debts.map((debt) => (
                  <div key={debt.name} className="grid grid-cols-2 border-t border-slate-200 bg-white dark:border-[#C0CAD6] dark:bg-white">
                    <div className="px-5 py-4 font-sans text-sm font-semibold text-slate-900 dark:text-[#4E537D]">{debt.name}</div>
                    <div className="px-5 py-4 text-sm text-slate-600 dark:text-slate-600">{debt.concern}</div>
                  </div>
                ))}
              </div>
            </section>
            ...
            <section className="mb-12">
              ...
              <p className="mt-5 font-sans text-sm font-semibold text-slate-500 dark:text-slate-600">Part 4 will explore that connection.</p>
            </section>
