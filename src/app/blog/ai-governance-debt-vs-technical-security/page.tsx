      ...
      <Container>
        <article className="mx-auto max-w-4xl">
          <header className="mb-10 border-b border-slate-200 pb-9">
            ...
            <p className="mb-3 font-sans text-xs font-semibold text-slate-500">PART 3 · AI GOVERNANCE DEBT SERIES</p>
            <h1 className="max-w-4xl font-sans text-3xl font-bold leading-[1.1] tracking-[-0.025em] text-slate-950 sm:text-4xl lg:text-5xl">
              <span className="underline decoration-amber-300 decoration-4 underline-offset-4">AI Governance Debt</span> vs Technical Debt vs Security Debt
            </h1>
          ...
          <div className="font-serif text-base leading-7 text-slate-700 sm:text-[17px] sm:leading-8">
            ...
            <section className="mb-12 grid gap-5 md:grid-cols-3">
              {debts.map((debt, index) => (
                <article key={debt.name} className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                  <div className="mb-4 flex h-8 w-8 items-center justify-center rounded-full bg-slate-100 font-sans text-xs font-bold text-slate-600">
                    {index + 1}
                  </div>
                  <h2 className="font-sans text-lg font-bold leading-7 text-slate-950">{debt.name}</h2>
                  <p className="mt-3 text-sm leading-6 text-slate-600">{debt.question}</p>
                </article>
              ))}
            </section>
            ...
            <section className="mb-12">
              ...
              <div className="rounded-2xl border border-amber-200 bg-amber-50 p-6 sm:p-7">
                <p className="mb-4 font-sans text-lg font-bold tracking-tight text-amber-950 sm:text-xl">
                  And still carry high <DebtHighlight />:
                </p>
                <ul className="list-disc space-y-2 pl-5 text-sm leading-6 text-slate-700 marker:text-amber-700 sm:text-base sm:leading-7">
            ...
            <section className="mb-12">
              ...
              <ol className="space-y-5 pl-6">
                <li className="pl-2">
                  <strong className="font-sans font-bold text-slate-950">It&apos;s portfolio-wide.</strong> An organization may run hundreds of AI systems, models and third-party AI services — each accumulating its own gaps.
                </li>
                <li className="pl-2">
                  <strong className="font-sans font-bold text-slate-950">It&apos;s dynamic.</strong> Models change, regulations change, use cases change, risk profiles change — governance has to track a moving target.
                </li>
                <li className="pl-2">
                  <strong className="font-sans font-bold text-slate-950">It compounds.</strong> If governance capability doesn&apos;t scale as fast as the AI portfolio grows, unresolved obligations pile up faster than the organization can close them.
                </li>
              </ol>
            </section>
            ...
            <section className="mb-12">
              <h2 className="mb-5 font-sans text-xl font-bold tracking-tight text-slate-950 sm:text-2xl">
                A simple management model
              </h2>
              <div className="overflow-hidden rounded-2xl border border-slate-200">
                <div className="grid grid-cols-2 bg-slate-50 font-sans text-xs font-bold uppercase tracking-wider text-slate-500">
                  <div className="px-5 py-3">Debt type</div>
                  <div className="px-5 py-3">Management concern</div>
                </div>
                {debts.map((debt) => (
                  <div key={debt.name} className="grid grid-cols-2 border-t border-slate-200 bg-white">
                    <div className="px-5 py-4 font-sans text-sm font-semibold text-slate-900">{debt.name}</div>
                    <div className="px-5 py-4 text-sm text-slate-600">{debt.concern}</div>
                  </div>
                ))}
              </div>
            </section>
            ...
            <section className="mb-12">
              ...
              <p className="mt-5 font-sans text-sm font-semibold text-slate-500">Part 4 will explore that connection.</p>
            </section>
            ...
            <aside className="mt-8 rounded-xl bg-slate-50 p-4 font-sans text-xs leading-6 text-slate-600">
              <strong className="text-slate-900">Author note:</strong> The term “AI Governance Debt” is used here as a proposed conceptual construct for personal research. It is not presented as an official NIST or ISO term.
            </aside>
          </div>
        </article>
      </Container>
