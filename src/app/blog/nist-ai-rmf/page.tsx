  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:p-7">
      <div className="flex items-start gap-4">
        <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-slate-100 font-sans text-xs font-bold text-slate-600 dark:bg-slate-800 dark:text-slate-300">{item.number}</span>
        <div>
          <h2 className="font-sans text-xl font-bold tracking-tight text-slate-950 dark:text-white sm:text-2xl">{item.title}</h2>
      ...
      <Container>
        <article className="mx-auto max-w-4xl">
          <header className="mb-10 border-b border-slate-200 pb-9 dark:border-slate-800">
            <p className="mb-4 font-sans text-[11px] font-bold uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">AI Governance · Responsible AI · AI Risk Management</p>
            <h1 className="max-w-4xl font-sans text-3xl font-bold leading-[1.1] tracking-[-0.025em] text-slate-950 dark:text-white sm:text-4xl lg:text-5xl">NIST AI Risk Management Framework: <span className="underline decoration-cyan-300 decoration-4 underline-offset-4 dark:decoration-cyan-500/70">GOVERN · MAP · MEASURE · MANAGE</span></h1>
            <p className="mt-5 max-w-3xl font-serif text-base leading-7 text-slate-600 dark:text-slate-300 sm:text-lg sm:leading-8">A practical perspective on turning AI risk management from a framework into a continuous organizational capability.</p>
            <div className="mt-6 flex flex-wrap gap-3 font-sans text-[11px] font-medium text-slate-500 dark:text-slate-400"><span className="rounded-full bg-slate-100 px-3 py-1.5 dark:bg-slate-900">Research &amp; Perspectives</span><span className="rounded-full bg-slate-100 px-3 py-1.5 dark:bg-slate-900">September 2026</span></div>
          </header>

          <div className="font-serif text-base leading-7 text-slate-700 dark:text-slate-300 sm:text-[17px] sm:leading-8">
            <p className="mb-7 border-l-4 border-cyan-600 pl-5 text-lg font-semibold leading-7 text-slate-950 dark:border-cyan-400 dark:text-white sm:text-xl sm:leading-8">As organizations move from experimenting with AI to deploying it at scale, the question is no longer whether AI risk exists. The question is how an organization builds the capability to manage it consistently.</p>
            <p className="mb-7">The NIST AI Risk Management Framework (AI RMF) 1.0 provides a structured way to think about AI risk across the lifecycle. At its core are four functions: <strong className="font-sans font-bold text-slate-950 dark:text-white">GOVERN, MAP, MEASURE and MANAGE</strong>. These functions are complementary rather than a simple sequence, with GOVERN providing the organizational foundation that remains relevant across the lifecycle.</p>
            ...
            <section className="mb-12 rounded-2xl border border-slate-200 bg-slate-50 p-6 dark:border-slate-800 dark:bg-slate-900/60 sm:p-7">
              <h2 className="font-sans text-xl font-bold tracking-tight text-slate-950 dark:text-white sm:text-2xl">The four functions at a glance</h2>
              <div className="mt-6 grid gap-3 sm:grid-cols-4">{functions.map((item) => <div key={item.title} className="rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-950"><p className="font-sans text-xs font-bold text-slate-400">{item.number}</p><p className="mt-2 font-sans text-sm font-bold text-slate-950 dark:text-white">{item.title}</p></div>)}</div>
            </section>

            <div className="space-y-6">{functions.map((item) => <FunctionCard key={item.title} item={item} />)}</div>

            <figure className="my-12">
              <div className="overflow-hidden rounded-2xl border border-slate-200 bg-slate-50 p-2 shadow-sm dark:border-slate-800 dark:bg-slate-900"><Image src="/nist-ai-rmf-functions.svg" alt="Conceptual view of the NIST AI Risk Management Framework four functions: Govern, Map, Measure and Manage." width={1000} height={620} className="h-auto w-full rounded-xl" /></div>
              <figcaption className="mt-3 text-center font-sans text-xs leading-5 text-slate-500 dark:text-slate-400">A practical view of the four NIST AI RMF functions and their relationship across the AI lifecycle.</figcaption>
            </figure>

            <section className="mb-12 border-y border-slate-200 py-10 dark:border-slate-800">
              <h2 className="mb-5 font-sans text-xl font-bold tracking-tight text-slate-950 dark:text-white sm:text-2xl">From framework to continuous capability</h2>
              ...
              <div className="rounded-2xl bg-slate-950 px-5 py-6 text-center font-sans font-bold text-white dark:bg-white dark:text-slate-950 sm:px-7 sm:py-7 sm:text-lg">MAP → MEASURE → MANAGE → LEARN → MAP</div>
              <p className="mt-5 text-sm leading-6 text-slate-600 dark:text-slate-400">The <strong className="font-sans">LEARN</strong> step is my practical interpretation of the organizational feedback loop, not a fifth NIST AI RMF function. It represents learning from incidents, monitoring, evaluation results and changes in context before the next MAP cycle.</p>
            </section>

            <section className="mb-12">
              <h2 className="mb-4 font-sans text-xl font-bold tracking-tight text-slate-950 dark:text-white sm:text-2xl">Why this matters for AI Governance Debt</h2>
            ...
            <section className="mb-12 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
              <div className="p-6 sm:p-7">
                ...
                <h2 className="mt-2 font-sans text-xl font-bold tracking-tight text-slate-950 dark:text-white sm:text-2xl">NIST AI RMF — GOVERN · MAP · MEASURE · MANAGE</h2>
              ...
              <div className="border-t border-slate-200 bg-slate-50 p-6 dark:border-slate-800 dark:bg-slate-950/60">
                ...
                <p className="mt-3 font-sans text-xs text-slate-500 dark:text-slate-400">The deck is presented here for viewing. Use the full-screen option for a larger reading experience.</p>
              </div>
            </section>

            <section className="mb-12 border-b border-slate-200 pb-10 dark:border-slate-800">
              <h2 className="mb-4 font-sans text-xl font-bold tracking-tight text-slate-950 dark:text-white sm:text-2xl">Questions I am exploring</h2>
            ...
            <section className="font-sans">
              ...
              <p className="mt-5 text-xs leading-6 text-slate-500 dark:text-slate-400">Note: NIST AI RMF 1.0 is currently being revised. This article and the accompanying presentation summarize AI RMF 1.0 and its current companion resources.</p>
            </section>

            <aside className="mt-8 rounded-xl bg-slate-50 p-4 font-sans text-xs leading-6 text-slate-600 dark:bg-slate-900/70 dark:text-slate-400"><strong className="text-slate-900 dark:text-slate-200">Author note:</strong> This article is a practical interpretation of NIST AI RMF 1.0. GOVERN, MAP, MEASURE and MANAGE are NIST terminology; the MAP → MEASURE → MANAGE → LEARN → MAP loop is presented as my own operational interpretation rather than an additional NIST function.</aside>
          </div>
        </article>
      </Container>
    </main>
